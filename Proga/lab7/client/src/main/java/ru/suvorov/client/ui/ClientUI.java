...
import ru.suvorov.server.commands.GenerateRandomObj;
...
import static ru.suvorov.server.commands.ExecuteScript.scriptStack;


public class ClientUI {
    public StadartConsole console;
    ...
    public ClientUI(ClientConnection connection) {
        ...
        this.running = true;
        this.runner = new Runner(console, new CollectionManager(), new DBConnector(), new DBManager(new DBConnector()));
    }



    public void start() {
        AuthorizationModule.askForRegistration();
        ...
        while (running) {
            try {
                ...
                if (commandName.equals("exit")) {
                    running = false;
                    System.out.println("DEBUG: Вырубаемся");
                ...
                if (commandName.equals("add")) {
                    ...
                    CommandRequest request = new CommandRequest(commandName, city, App.currentUsername, App.currentUserPass);
                ...
                if (commandName.equals("update_id")) {
                    ...
                    Object response = connection.sendRequest(request);
                    if (response instanceof ExecutionResponse) {
                        console.println(runner.getCommands().get("update_id").apply(id, request));
                    }
                    continue;
                }

                if (commandName.equals("generate_random_obj")) {
                    CommandRequest request = new CommandRequest(commandName, argument, App.currentUsername, App.currentUserPass);
                    Object response = connection.sendRequest(request);
                    if (response == null) {
                        console.printError("Нет ответа от сервера\n");
                    } else if (response instanceof ExecutionResponse) {
                ...
                if (commandName.equals("print_ascending")) {
                    CommandRequest request = new CommandRequest(commandName, new City());
                    Object response = connection.sendRequest(request);

                    if (response instanceof ExecutionResponse) {
                        runner.getCommands().get("print_ascending").apply(new City(), request);
                        console.println(((ExecutionResponse) response).getMessage());
                ...
                if (commandName.equals("execute_script")) {
                    File file = new File("/Users/n0fckgway/Desktop/University_ITMO/Proga/lab7/script");
                    String fileName = file.getPath();
                    CommandRequest request = new CommandRequest(commandName, fileName);
                    Object response = connection.sendRequest(request);
                    if (response instanceof ExecutionResponse) {
                        console.println(runner.getCommands().get("execute_script").apply(fileName, request));
                    }
                }

                if (commandName.equals("info")) {
                    CollectionManager collectionManager = new CollectionManager();
                    CommandRequest request = new CommandRequest(commandName, collectionManager.info());
                    Object response = connection.sendRequest(request);
                    if (response instanceof ExecutionResponse) {
                        console.println(runner.getCommands().get("info").apply(collectionManager.info(), request));
                ...
                if (commandName.equals("clear")) {
                    CommandRequest request = new CommandRequest(commandName, new City(), App.currentUsername, App.currentUserPass);
                    Object response = connection.sendRequest(request);
                    if (response instanceof ExecutionResponse) {
                        console.println(((ExecutionResponse) response).getMessage());
                    }
                ...
                if (commandName.equals("show")) {
                    ...
                    if (response instanceof ExecutionResponse) {
                        console.println(((ExecutionResponse) response).getMessage());
                    } else if (response instanceof ConcurrentLinkedQueue) {
                        console.println("Коллекция:");
                        for (Object obj : (ConcurrentLinkedQueue<?>) response) {
                            console.println(obj);
                        }
                    } else {
                        console.printError("Неизвестный тип ответа от сервера для команды show: " + (response != null ? response.getClass().getName() : "null") + "\n");
                    }
                    continue;
                }
    ...
    private static City readCityFromConsole(StadartConsole console) {
        System.out.println("DEBUG: Начало ввода города");
        ...
        String capital;
        while (true) {
            console.print("Столица (может быть пустой): ");
            try {
                capital = console.readln();
                if ((capital == null) || capital.isEmpty()) {
                    capital = "-";
                }
                break;
            } catch (Exception e) {
                console.printError("Введите строку!\n");
            }
        }
        ...
        city.setCreationDate(java.time.ZonedDateTime.now());
        city.setOwnerUsername(App.currentUsername);
        return city;
    }
} 