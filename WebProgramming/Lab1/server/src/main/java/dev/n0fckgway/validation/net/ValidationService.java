package dev.n0fckgway.validation.net;

import com.fastcgi.FCGIInterface;
import dev.n0fckgway.fcgi.FCGIService;
import dev.n0fckgway.validation.net.api.dots.Dot;
import dev.n0fckgway.validation.net.api.dots.DotsRequest;
import dev.n0fckgway.validation.net.api.dots.DotsResponse;
import dev.n0fckgway.validation.net.api.hit.HitRequest;
import dev.n0fckgway.validation.net.api.hit.HitResponse;
import dev.n0fckgway.validation.net.parse.ParseException;
import dev.n0fckgway.validation.net.parse.ParseQuery;
...
public class ValidationService implements Runnable {
    private final FCGIService fcgiService = new FCGIService();
    ...
    private void processHit() throws ParseException {
        ...
        Map<String, String> queries = ParseQuery.parse(FCGIInterface.request.params.getProperty("QUERY_STRING"));
        if (queries.size() < 3) {
    ...
    private void processDots() throws ParseException {
        ...
        List<HitRequest> keyList = new ArrayList<>(cache.keySet());
        for (HitRequest rq : keyList.subList(from, to)) {
